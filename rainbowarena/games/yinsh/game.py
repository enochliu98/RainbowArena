    ...
    def get_payoffs(self):
        ...
        if winner is not None:
            for idx in range(self.num_players):
                self.payoffs[idx] = self.players[idx].point
            min_val = min(self.payoffs)
            max_val = max(self.payoffs)
            self.payoffs = [(payoff - min_val) / (max_val - min_val + 0.00001) - 0.5 for payoff in self.payoffs]
