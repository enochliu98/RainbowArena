...
class Env(gym.Env):
    ...
    def step_tmp(self, action, raw_action=False):
        ''' Step forward
    ...
    def reset(self):
        '''
            适用于gym的reset
        '''
        next_state, player_id = self.reset_tmp()
        info = {}

        while player_id != 0:
            action = self.agents[player_id].step(next_state)  # 其他玩家执行动作
            next_state, player_id = self.step_tmp(action)
    ...
    def step(self, action):
        ...
        # 这里假设智能体对应的玩家为0
        # 其他玩家+游戏还没结束
        while player_id != 0 and done is False:
            action = self.agents[player_id].step(next_state)  # 其他玩家执行动作
            next_state, player_id = self.step_tmp(action)
