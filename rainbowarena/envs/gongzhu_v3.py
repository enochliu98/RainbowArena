...
class GongzhuEnv(Env):
    ...
    def __init__(self, config):
        ...
        隐藏信息，对手的手牌 (52 * 3)
        """
        self.state_invisible_info_len = 52 * 3

        # 上述相加
        self.state_len = self.state_player_len + self.state_deck_len + self.state_invisible_info_len
    ...
    def _extract_state(self, state):
        ...
        idxs = [
            ...
            self.state_player_len + self.state_deck_len + self.state_invisible_info_len
        ]

        obs[: idxs[0]] = extract_player_state(state)
        obs[idxs[0]: idxs[1]] = extract_deck_state(state)
        obs[idxs[1]: idxs[2]] = extract_invisible_info_state(state)
...
def extract_invisible_info_state(state):
    invisible_info_state = []

    invisible_info_state.extend(state["opponent_cards"][0])
    invisible_info_state.extend(state["opponent_cards"][1])
    invisible_info_state.extend(state["opponent_cards"][2])

    return np.array(invisible_info_state)
