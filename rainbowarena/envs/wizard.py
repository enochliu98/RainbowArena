...
from rainbowarena.games.wizard import Game
...
class WizardEnv(Env):
    ...
    def __init__(self, config):
        ...
        self.state_duns_prd_player_len = 1
        self.state_duns_player_len = 1
        self.state_unique_len = 4
        # 玩家信息
        self.state_player_len = self.state_cards_player_len + \
                                self.state_point_player_len * 4 + \
                                self.state_duns_prd_player_len * 4 + \
                                self.state_duns_player_len * 4
        ...
        """
        # 桌面功能卡
        self.state_ace_color_deck_len = 1
        ...
        # 桌面信息
        self.state_deck_len = self.state_ace_color_deck_len + \
                              self.state_dun_color_deck_len + \
...
def extract_player_state(state):
    ...
    player_state.extend(extract_cards_state(state))
    player_state.append(state["self_point"])
    player_state.append(state["self_duns"])
    player_state.append(state["self_duns_prd"])
    player_state.extend(state["opponent_point"])
    player_state.extend(state["opponent_duns"])
    player_state.extend(state["opponent_duns_prd"])
    # player_state.extend(extract_unique_state(state, 4))
...
def extract_deck_state(state):
    ...
    deck_state.append(state["deck_ace_color"])
    deck_state.append(state["deck_dun_color"])
    deck_state.extend(extract_dun_cards_state(state))
...
def extract_dun_cards_state(state):
    ...
    提取dun中卡特征
    """
    state_dun_cards = [dun_card[1] for dun_card in state["deck_dun_cards"]]
